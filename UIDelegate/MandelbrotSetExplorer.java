...
/**
 * The MandelbrotSetExplorer class contains method to establish the GUI and
 * interact with users. example usage: new MandelbrotSetExplorer();
 * 
 * @author 180007800
 *
 */
public class MandelbrotSetExplorer extends JComponent {
	...
	@Override
	/**
	 * Override paint method. Paint the image generated from the Model part. Paint
	 * the square used to indicate the part to zoom in. paint the line used to
	 * indicate the direction to pan in.
	 * 
	 * @param g
	 */
...
//			}
//		}
//		//painting the line generated by the location where mouse clicked (x1, y1) and dragged to (x2, y2).
//		if(isPan == true && isZoom == false) {
//			if(!released) {
//				if (!pointList.isEmpty()) {
//					int x1 = pointList.get(0)[0];
//					int y1 = pointList.get(0)[1] - MENU_WIDTH;
...
//		}
	...
	/**
	 * Setup all components of the main frame: the menu set, the tool bar, mouse
	 * listeners, and the main canvas of Mandelbrot Set
	 */
	private void setup() {
		setupMenu();
		setupToolbar();
	...
	/**
	 * Setup the menu set with the file pull-down menu: load data, save data, and
	 * save image.
	 */
	...
	/**
	 * Setup the frame with all buttons: reset, change colour, undo, redo, a text
	 * field to change max iteration, and a button to update with the given max
	 * iteration.
	 */
	private void setupToolbar() {
		...
		resetButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				textfield
						.setText("" + modelCalculator.getData().get(modelCalculator.getStateIndex()).getMaxIteration());
				frame.repaint();
			}
		});
		...
		undoButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (modelCalculator.getStateIndex() > 0) {
					...
					frame.repaint();
					textfield.setText(
							"" + modelCalculator.getData().get(modelCalculator.getStateIndex()).getMaxIteration());
				}
				flagUndo = true;
			}
		});
	...
	/**
	 * The method implemented to pan around with given line.
	 * 
	 * @param x1
	 * @param x2
	 * @param y1
	 * @param y2
	 */
	private void pan(double x1, double x2, double y1, double y2) {
		Pan p = new Pan(x1, x2, y1, y2);
		if (flagUndo) {
			modelCalculator.removeTail();
			flagUndo = false;
		}
		// generate a new state of Mandelbrot set to pan in.
		if (x1 != x2 || y1 != y2) {
			modelCalculator.setNextState(p);
